...
@section('content')
    <div class="space-y-6">
        <h1 class="text-3xl font-bold text-gray-900">Pengajuan Perolehan Kredit</h1>

        <!-- Filter Section -->
        <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('admin.pengajuan.perolehan-kredit') }}" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
                    <!-- Search -->
                    <div class="lg:col-span-2">
                        <label for="search" class="block text-sm font-medium text-gray-700">Cari (Nama/No. Bukti)</label>
                        <input
                            type="text"
                            id="search"
                            name="search"
                            value="{{ $filters['search'] ?? '' }}"
                            placeholder="Cari nama atau no. bukti..."
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($programStudis as $prodi)
                                <option value="{{ $prodi->id }}" {{ ($filters['program_studi_id'] ?? '') === $prodi->id ? 'selected' : '' }}>
                                    {{ $prodi->nama_prodi }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
                    ...
                    <div class="flex items-end gap-2 lg:col-span-4">
                        ...
                        <a
                            href="{{ route('admin.pengajuan.perolehan-kredit') }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">No. Bukti</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse($pengajuan as $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm">
                                    <a
                                        href="{{ route('admin.pengajuan.perolehan-kredit.show', $item->id) }}"
